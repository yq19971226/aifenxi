...
export default function OperatorsPage() {
  ...
  const fetchOperators = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : t("operators.fetchFailed"));
    } finally {
      setLoading(false);
    }
  }, [t]);
  ...
  const handleCreate = useCallback(
    async (e: React.FormEvent) => {
      ...
      if (password.length < 8) {
        setFormError(t("operators.passwordMinLength"));
        return;
      }
      ...
      try {
        ...
        setFormError(err instanceof Error ? err.message : t("operators.createFailed"));
      } finally {
        setCreating(false);
      }
    },
    [email, password, fetchOperators, t],
  );
  ...
  const handleToggle = useCallback(
    async (op: OperatorInfo) => {
      ...
      try {
        ...
        setError(err instanceof Error ? err.message : t("operators.operationFailed"));
      } finally {
        setTogglingId(null);
      }
    },
  ...
  if (!isAdmin) {
    return (
      <div className="flex min-h-[50vh] items-center justify-center">
        <p className="text-sm text-zinc-500">{t("operators.noAccess")}</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-6 p-6">
      ...
      >
        ...
        >
          {creating ? t("operators.creating") : t("operators.createButton")}
        </button>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-white/[0.08] text-left text-xs text-zinc-500">
                  <th className="px-5 py-3 font-medium">{t("operators.email")}</th>
                  <th className="px-5 py-3 font-medium">{t("operators.createdAt")}</th>
                  <th className="px-5 py-3 font-medium">{t("operators.status")}</th>
                  <th className="px-5 py-3 font-medium text-right">{t("operators.action")}</th>
                </tr>
              </thead>
              <tbody>
                {operators.map((op) => (
