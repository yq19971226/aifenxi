...
export function AnalysisStatusBanner({ report }: { report: AnalysisReportType }) {
  const t = useTranslations("analysis.statusBanner");
  ...
  const statusConfig: Record<string, { border: string; bg: string; text: string; label: string; icon: typeof Check }> = {
    actionable: { border: "border-emerald-500/20", bg: "bg-emerald-500/[0.04]", text: "text-emerald-400", label: t("actionable"), icon: Check },
    degraded: { border: "border-amber-500/20", bg: "bg-amber-500/[0.04]", text: "text-amber-400", label: t("degraded"), icon: AlertTriangle },
    blocked: { border: "border-red-500/20", bg: "bg-red-500/[0.04]", text: "text-red-400", label: t("blocked"), icon: X },
  };
